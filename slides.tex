	...
	\begin{minipage}[t][.7\textheight]{\textwidth}
		\only<1>{\pdfcomment{As can be seen, our conversation analogy (and therefore our expectations) translates badly to the world of messaging. We need a new analogy. -- transition to next slide}
			...
			\begin{itemize}
				...
				\item not guarantee any secrecy, so we have neither \alert{forward secrecy} nor \alert{future secrecy}
...
\begin{frame}
	...
	\begin{tabular}{@{}p{.6\textwidth}p{.4\textwidth}}
		...
		\parbox{.6\textwidth}{\alert{Key Agreement} allows us to create symmetric keys based on asymmetric key pairs.} & \parbox{.4\textwidth}{\includegraphics[width=.4\textwidth]{img/pka.pdf}} \\
	\end{tabular}
	\vfill
