		...
		\begin{columns}
			...
			\begin{column}[t]{.69\textwidth}
				\begin{overlayarea}{\textwidth}{\textheight}
					...
					\only<1>{%
						Exchange a set of \aert{ephemral keys} and verify each others long term identity keys.
					}
					\only<2>{%
					...
					\only<4>{%
						...
						\begin{itemize}
							\item Outer Encryption with \aert{ephemeral Keys}
							\item Ciphertext links clients \aert{ephemeral key} pair to it's long term key pair
						\end{itemize}
					}	
					\only<5>{%
						...
						\begin{itemize}
							\item Server comfirms everything worked fine by encrypting something with both \alrt{ephemeral keys}
